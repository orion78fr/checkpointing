...
public class App implements EDProtocol {
	...
	private void doRollback(Message msg) {
		...
		if (received[from] > msg.getMsg()) {
			...
			do {
				c = checkpoints.pop();
			} while (c.getReceived()[from] > msg.getMsg()); // Looking for a
															// consistent
															// rollback

			checkpoints.push(c); // The checkpoint is still valid, so we can
									// reuse it later
	...
	private void planNextCheckpoint() {
		// Next checkpoint
		if (!Constants.isDomino()) { // If domino, we force checkpoint on
										// message reception
